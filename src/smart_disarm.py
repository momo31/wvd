...
class DisarmConfig:
    ...
    stop_time = 0.57           # 탭 주입~정지까지 시간 추정(s). 초기값 1.0은 유저 육안 실측이었으나
                               # 2프레임 감속 역산 실측 누적(26.07-16~18 0.31~0.71s n=46,
                               # 26.07-28 0.47~0.64s median 0.57 n=21)에 맞춰 0.57로 재설정.
                               # 기반 리드(stop_time/2)가 0.500→0.285로 내려와 세션 수렴 리드
                               # 실측(0.187~0.328)이 adj ±0.10 안에 들어온다. 1.0 시절엔 매 세션
                               # adj가 -0.3 부근(상한 -0.35)까지 끌려가 보정 여유가 소진되고
                               # 세션 초반이 워밍업 미스로 소모됐다(26.07-28 19시대 성공률 25%).
    ...
    ts_solve_max = 1.2           # 상한(s). 초과면 등속(탭 미반영/신규 커서) 의심 → 측정 폐기.
                                 # 반사 가드의 최대 주행거리 산정에도 쓰므로 실측 상한에 여유만
                                 # 두고 낮게 유지한다(높이면 가드가 과보수화 → 측정 기아).
                                 # 26.07-16~18 실측 개체 감속 분포 0.31~0.71s (n=46) 기반.
